...
#pragma omp parallel
    ...
    exc.Rethrow();
    this->histogram_builder_->Reset(nbins, param_.max_bin, this->nthread_, 1,
    ...
    const bool is_lossguide = static_cast<TrainParam::TreeGrowPolicy>(param_.grow_policy) !=
                              TrainParam::kDepthWise;
    opt_partition_builder_.Init<BinIdxType>(gmat, column_matrix, &tree,
                                            this->nthread_, param_.max_depth,
                                            is_lossguide);

    const size_t block_size = common::GetBlockSize(info.num_row_, this->nthread_);
